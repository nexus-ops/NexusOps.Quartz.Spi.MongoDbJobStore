...
namespace Quartz.Util
{
    ...
    public static class LogicalThreadContext
    {
        ...
        {
            return _state.TryGetValue(name, out AsyncLocal<object>? data) ? (T)data.Value! : default;
        }
        ...
        {
            _state.GetOrAdd(name, _ => new AsyncLocal<object>()).Value = value;
        }
        ...
        {
            _state.TryRemove(name, out AsyncLocal<object>? discard);
        }
    }
}
